# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'calculatoruiupgrade.ui'
#
# Created by: PyQt5 UI code generator 5.15.6
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.
import math

from PyQt5 import QtCore, QtGui, QtWidgets

import ekuacione_fuqi2
import ekuacione_fuqi3
import ekuacione_fuqi1
import grafiket_fuqi1
from shikozgjidhjet2 import Ui_Zgjidhje
import grafiket_fuqi2
import grafiket_fuqi3


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(294, 517)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../../Downloads/mathematics-operation-addition-manic-math-multiplication-and-division-flashcard-math-games-mathematics.jpg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        MainWindow.setWindowIcon(icon)
        MainWindow.setStyleSheet("background-color: rgb(13, 20, 28);")
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.centralwidget)
        self.verticalLayout.setObjectName("verticalLayout")
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setObjectName("gridLayout")
        self.push_2 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_2.sizePolicy().hasHeightForWidth())
        self.push_2.setSizePolicy(sizePolicy)
        self.push_2.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_2.setObjectName("push_2")
        self.gridLayout.addWidget(self.push_2, 7, 1, 1, 1)
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.eqinfotxt = ""
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(13)
        sizePolicy.setHeightForWidth(self.label.sizePolicy().hasHeightForWidth())
        self.label.setSizePolicy(sizePolicy)
        self.label.setStyleSheet("font: 60 18pt \"MS Shell Dlg 2\";\n"
"\n"
"color: rgb(255, 255, 255);")
        self.label.setText("")
        self.label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 0, 0, 1, 4)
        self.push_zero = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_zero.sizePolicy().hasHeightForWidth())
        self.push_zero.setSizePolicy(sizePolicy)
        self.push_zero.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_zero.setObjectName("push_zero")
        self.gridLayout.addWidget(self.push_zero, 8, 0, 1, 1)
        self.push_div = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_div.sizePolicy().hasHeightForWidth())
        self.push_div.setSizePolicy(sizePolicy)
        self.push_div.setStyleSheet("QPushButton {\n"
" font: 75 26pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_div.setObjectName("push_div")
        self.gridLayout.addWidget(self.push_div, 7, 3, 1, 1)
        self.push_8 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_8.sizePolicy().hasHeightForWidth())
        self.push_8.setSizePolicy(sizePolicy)
        self.push_8.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_8.setObjectName("push_8")
        self.gridLayout.addWidget(self.push_8, 5, 1, 1, 1)
        self.push_plus = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_plus.sizePolicy().hasHeightForWidth())
        self.push_plus.setSizePolicy(sizePolicy)
        self.push_plus.setStyleSheet("QPushButton {\n"
" font: 75 30pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_plus.setObjectName("push_plus")
        self.gridLayout.addWidget(self.push_plus, 4, 3, 1, 1)
        self.push_7 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_7.sizePolicy().hasHeightForWidth())
        self.push_7.setSizePolicy(sizePolicy)
        self.push_7.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_7.setObjectName("push_7")
        self.gridLayout.addWidget(self.push_7, 5, 0, 1, 1)
        self.push_point = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_point.sizePolicy().hasHeightForWidth())
        self.push_point.setSizePolicy(sizePolicy)
        self.push_point.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_point.setObjectName("push_point")
        self.gridLayout.addWidget(self.push_point, 8, 1, 1, 1)
        self.push_9 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_9.sizePolicy().hasHeightForWidth())
        self.push_9.setSizePolicy(sizePolicy)
        self.push_9.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_9.setObjectName("push_9")
        self.gridLayout.addWidget(self.push_9, 5, 2, 1, 1)
        self.push_4 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_4.sizePolicy().hasHeightForWidth())
        self.push_4.setSizePolicy(sizePolicy)
        self.push_4.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_4.setObjectName("push_4")
        self.gridLayout.addWidget(self.push_4, 6, 0, 1, 1)
        self.push_xbtn = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_xbtn.sizePolicy().hasHeightForWidth())
        self.push_xbtn.setSizePolicy(sizePolicy)
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(30)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(9)
        self.push_xbtn.setFont(font)
        self.push_xbtn.setStyleSheet("QPushButton {\n"
" font: 75 26pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_xbtn.setObjectName("push_xbtn")
        self.gridLayout.addWidget(self.push_xbtn, 4, 1, 1, 1)
        self.push_1 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_1.sizePolicy().hasHeightForWidth())
        self.push_1.setSizePolicy(sizePolicy)
        self.push_1.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_1.setObjectName("push_1")
        self.gridLayout.addWidget(self.push_1, 7, 0, 1, 1)
        self.push_3 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_3.sizePolicy().hasHeightForWidth())
        self.push_3.setSizePolicy(sizePolicy)
        self.push_3.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_3.setObjectName("push_3")
        self.gridLayout.addWidget(self.push_3, 7, 2, 1, 1)
        self.push_fuqi = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_fuqi.sizePolicy().hasHeightForWidth())
        self.push_fuqi.setSizePolicy(sizePolicy)
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(30)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(9)
        self.push_fuqi.setFont(font)
        self.push_fuqi.setStyleSheet("QPushButton {\n"
" font: 75 26pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_fuqi.setObjectName("push_fuqi")
        self.gridLayout.addWidget(self.push_fuqi, 4, 2, 1, 1)
        self.push_6 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_6.sizePolicy().hasHeightForWidth())
        self.push_6.setSizePolicy(sizePolicy)
        self.push_6.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_6.setObjectName("push_6")
        self.gridLayout.addWidget(self.push_6, 6, 2, 1, 1)
        self.push_del = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_del.sizePolicy().hasHeightForWidth())
        self.push_del.setSizePolicy(sizePolicy)
        self.push_del.setStyleSheet("QPushButton {\n"
" font: 75 16pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_del.setObjectName("push_del")
        self.gridLayout.addWidget(self.push_del, 4, 0, 1, 1)
        self.push_5 = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_5.sizePolicy().hasHeightForWidth())
        self.push_5.setSizePolicy(sizePolicy)
        self.push_5.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_5.setObjectName("push_5")
        self.gridLayout.addWidget(self.push_5, 6, 1, 1, 1)
        self.push_equal = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_equal.sizePolicy().hasHeightForWidth())
        self.push_equal.setSizePolicy(sizePolicy)
        self.push_equal.setStyleSheet("QPushButton {\n"
" font: 75 12pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(240, 37, 40);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(206, 33, 35);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_equal.setObjectName("push_equal")
        self.gridLayout.addWidget(self.push_equal, 8, 3, 1, 1)
        self.push_mult = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_mult.sizePolicy().hasHeightForWidth())
        self.push_mult.setSizePolicy(sizePolicy)
        self.push_mult.setStyleSheet("QPushButton {\n"
" font: 75 30pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_mult.setObjectName("push_mult")
        self.gridLayout.addWidget(self.push_mult, 6, 3, 1, 1)
        self.push_min = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_min.sizePolicy().hasHeightForWidth())
        self.push_min.setSizePolicy(sizePolicy)
        self.push_min.setStyleSheet("QPushButton {\n"
" font: 75 30pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(22, 26, 35);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(22, 29, 37);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_min.setObjectName("push_min")
        self.gridLayout.addWidget(self.push_min, 5, 3, 1, 1)
        self.push_baraz = QtWidgets.QPushButton(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(50)
        sizePolicy.setVerticalStretch(10)
        sizePolicy.setHeightForWidth(self.push_baraz.sizePolicy().hasHeightForWidth())
        self.push_baraz.setSizePolicy(sizePolicy)
        self.push_baraz.setStyleSheet("QPushButton {\n"
" font: 75 25pt \"Calibri\";     \n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(13, 20, 28);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" max-width:64px;\n"
" max-height:64px;\n"
" min-width:64px;\n"
" min-height:64px;\n"
"}\n"
"\n"
"QPushButton:hover {\n"
" background-color: rgb(22, 26, 35);\n"
" color: rgb(255, 255, 255);\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(12, 16, 25);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_baraz.setObjectName("push_baraz")
        self.gridLayout.addWidget(self.push_baraz, 8, 2, 1, 1)
        self.push_shiko_zgjidhjen_btn = QtWidgets.QPushButton(self.centralwidget)
        self.push_shiko_zgjidhjen_btn.setStyleSheet("QPushButton {\n"
" font: 75 10pt \"Calibri\";\n"
" color: rgb(255, 255, 255);\n"
" background-color: rgb(147, 36, 50);\n"
" border-style: solid;\n"
" border-radius:32px;\n"
" min-height: 30px;\n"
" max-height: 30px;\n"
"}\n"
"\n"
"QPushButton:pressed {\n"
"    background-color: rgb(206, 33, 35);\n"
"    color: rgb(138, 142, 145);\n"
"}")
        self.push_shiko_zgjidhjen_btn.setObjectName("push_shiko_zgjidhjen_btn")
        self.gridLayout.addWidget(self.push_shiko_zgjidhjen_btn, 1, 1, 3, 2)
        self.verticalLayout.addLayout(self.gridLayout)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 294, 22))
        self.menubar.setObjectName("menubar")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

        self.push_1.clicked.connect(self.method_1)
        self.push_2.clicked.connect(self.method_2)
        self.push_3.clicked.connect(self.method_3)
        self.push_4.clicked.connect(self.method_4)
        self.push_5.clicked.connect(self.method_5)
        self.push_6.clicked.connect(self.method_6)
        self.push_7.clicked.connect(self.method_7)
        self.push_8.clicked.connect(self.method_8)
        self.push_9.clicked.connect(self.method_9)
        self.push_zero.clicked.connect(self.method_zero)
        self.push_point.clicked.connect(self.method_point)
        self.push_plus.clicked.connect(self.method_plus)
        self.push_min.clicked.connect(self.method_min)
        self.push_mult.clicked.connect(self.method_mult)
        self.push_div.clicked.connect(self.method_div)
        self.push_equal.clicked.connect(self.method_equal)
        self.push_del.clicked.connect(self.method_del)
        self.push_fuqi.clicked.connect(self.method_ngritja_ne_fuqi)
        self.push_xbtn.clicked.connect(self.method_x_button)
        self.push_shiko_zgjidhjen_btn.clicked.connect(self.method_shfaq_zgjidhjen)
        self.push_baraz.clicked.connect(self.method_baraz)

    def method_1(self):
            text = self.label.text()
            self.label.setText(text + "1")

    def method_2(self):
            text = self.label.text()
            self.label.setText(text + "2")

    def method_3(self):
            text = self.label.text()
            self.label.setText(text + "3")

    def method_4(self):
            text = self.label.text()
            self.label.setText(text + "4")

    def method_5(self):
            text = self.label.text()
            self.label.setText(text + "5")

    def method_6(self):
            text = self.label.text()
            self.label.setText(text + "6")

    def method_7(self):
            text = self.label.text()
            self.label.setText(text + "7")

    def method_8(self):
            text = self.label.text()
            self.label.setText(text + "8")

    def method_9(self):
            text = self.label.text()
            self.label.setText(text + "9")

    def method_zero(self):
            text = self.label.text()
            self.label.setText(text + "0")

    def method_point(self):
            text = self.label.text()
            self.label.setText(text + ".")

    def method_plus(self):
            text = self.label.text()
            self.label.setText(text + "+")

    def method_min(self):
            text = self.label.text()
            self.label.setText(text + "-")

    def method_mult(self):
            text = self.label.text()
            self.label.setText(text + "*")

    def method_x_button(self):
            text = self.label.text()
            self.label.setText(text + "x")

    def method_div(self):
            text = self.label.text()
            self.label.setText(text + "/")

    def method_ngritja_ne_fuqi(self):
            text = self.label.text()
            self.label.setText(text + "^")

    def method_baraz(self):
            text = self.label.text()
            self.label.setText(text + "=")


    def method_equal(self):
            text = self.label.text()
            try:
                    if ('^3' in text):
                            a = ekuacione_fuqi3.parse_eq(text)[0]
                            b = ekuacione_fuqi3.parse_eq(text)[1]
                            c = ekuacione_fuqi3.parse_eq(text)[2]
                            d = ekuacione_fuqi3.parse_eq(text)[3]
                            self.label.setText(ekuacione_fuqi3.solve(a,b,c,d))
                            grafiket_fuqi3.plot_me(a,b,c,d, 'r', text)
                    elif ('^2' in text):
                            self.eqinfotxt = text
                            a = ekuacione_fuqi2.parse_eq(text)[0]
                            b = ekuacione_fuqi2.parse_eq(text)[1]
                            c = ekuacione_fuqi2.parse_eq(text)[2]

                            self.label.setText(ekuacione_fuqi2.roots_of_equation(a, b, c))
                            self.push_shiko_zgjidhjen_btn.show()
                    elif ('x' in text):
                            self.eqinfotxt = text
                            a = ekuacione_fuqi1.parse_eq(text)[0]
                            b = ekuacione_fuqi1.parse_eq(text)[1]

                            self.label.setText(f"a={a} b={b}")
                            grafiket_fuqi1.plot_linear_eq(a,b)
                    else:
                            ans = eval(text)
                            self.label.setText(str(ans))
            except:
                    self.label.setText("Input Gabim")

    def method_del(self):
            text = self.label.text()
            self.label.setText(text[:len(text) - 1])
            self.push_shiko_zgjidhjen_btn.hide()

    def method_shfaq_zgjidhjen(self):
            self.window = QtWidgets.QMainWindow()
            self.ui = Ui_Zgjidhje()
            self.ui.setupUi(self.window)
            self.window.show()
            eq12 = self.eqinfotxt
            a = ekuacione_fuqi2.parse_eq(eq12)[0]
            b = ekuacione_fuqi2.parse_eq(eq12)[1]
            c = ekuacione_fuqi2.parse_eq(eq12)[2]
            self.ui.label.setText(eq12)
            self.ui.label_2.setText(f"a={a}, b={b}, c={c}")
            self.ui.label_3.setText("D=b^2-4ac")
            self.ui.label_4.setText(f"D={(b ** 2)} - {(4 * a * c)}")
            D = b ** 2 - 4 * a * c
            sqrt_D = math.sqrt(abs(D))
            self.ui.label_5.setText(f"D= {D}")
            self.ui.label_6.setText(f"x1= -({b})+√{D} ÷ 2*{a}")
            x1 = (-b + sqrt_D) / (2 * a)
            self.ui.label_7.setText(f"x2= -({b})-√{D} ÷ 2*{a}")
            x2 = (-b - sqrt_D) / (2 * a)
            self.ui.label_8.setText(f"x1={x1}, x2={x2}")
            grafiket_fuqi2.shfaq_grafikun(a,b,c)


    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "Calculator"))
        self.push_2.setText(_translate("MainWindow", "2"))
        self.push_2.setShortcut(_translate("MainWindow", "2"))
        self.push_zero.setText(_translate("MainWindow", "0"))
        self.push_zero.setShortcut(_translate("MainWindow", "0"))
        self.push_div.setText(_translate("MainWindow", "÷"))
        self.push_div.setShortcut(_translate("MainWindow", "/"))
        self.push_8.setText(_translate("MainWindow", "8"))
        self.push_8.setShortcut(_translate("MainWindow", "8"))
        self.push_plus.setText(_translate("MainWindow", "+"))
        self.push_plus.setShortcut(_translate("MainWindow", "+"))
        self.push_7.setText(_translate("MainWindow", "7"))
        self.push_7.setShortcut(_translate("MainWindow", "7"))
        self.push_point.setText(_translate("MainWindow", "."))
        self.push_point.setShortcut(_translate("MainWindow", "."))
        self.push_9.setText(_translate("MainWindow", "9"))
        self.push_9.setShortcut(_translate("MainWindow", "9"))
        self.push_4.setText(_translate("MainWindow", "4"))
        self.push_4.setShortcut(_translate("MainWindow", "4"))
        self.push_xbtn.setText(_translate("MainWindow", "x"))
        self.push_xbtn.setShortcut(_translate("MainWindow", "X"))
        self.push_1.setText(_translate("MainWindow", "1"))
        self.push_1.setShortcut(_translate("MainWindow", "1"))
        self.push_3.setText(_translate("MainWindow", "3"))
        self.push_3.setShortcut(_translate("MainWindow", "3"))
        self.push_fuqi.setText(_translate("MainWindow", "^"))
        self.push_fuqi.setShortcut(_translate("MainWindow", "^"))
        self.push_6.setText(_translate("MainWindow", "6"))
        self.push_6.setShortcut(_translate("MainWindow", "6"))
        self.push_del.setText(_translate("MainWindow", "⌫"))
        self.push_del.setShortcut(_translate("MainWindow", "Backspace"))
        self.push_5.setText(_translate("MainWindow", "5"))
        self.push_5.setShortcut(_translate("MainWindow", "5"))
        self.push_equal.setText(_translate("MainWindow", "Zgjidh"))
        self.push_equal.setShortcut(_translate("MainWindow", "Backspace"))
        self.push_mult.setText(_translate("MainWindow", "*"))
        self.push_mult.setShortcut(_translate("MainWindow", "*"))
        self.push_min.setText(_translate("MainWindow", "-"))
        self.push_min.setShortcut(_translate("MainWindow", "-"))
        self.push_baraz.setText(_translate("MainWindow", "="))
        self.push_baraz.setShortcut(_translate("MainWindow", "="))
        self.push_shiko_zgjidhjen_btn.setText(_translate("MainWindow", "Shiko Zgjidhjen"))
        self.push_shiko_zgjidhjen_btn.hide()


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
